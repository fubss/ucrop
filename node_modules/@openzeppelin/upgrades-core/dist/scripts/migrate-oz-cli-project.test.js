...
ava_1.default.before(async () => {
    ...
    migrationOutput = migrate_oz_cli_project_1.migrateManifestsData({ rinkeby: migratableData });
    ...
    await migrate_oz_cli_project_1.migrateLegacyProject();
});
ava_1.default.after(async () => {
    await rimraf(process.cwd());
});
ava_1.default('transforms manifest data', async (t) => {
    t.snapshot(migrationOutput.newManifestsData);
});
ava_1.default('produces network export data', async (t) => {
    t.snapshot(migrationOutput.networksExportData);
});
ava_1.default('deletes project file', async (t) => {
...
ava_1.default('export file contains networks and compiler data', async (t) => {
