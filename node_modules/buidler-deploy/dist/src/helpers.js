...
Diamond_json_1.default.abi = mergeABIs(false, Diamond_json_1.default.abi, DiamondFacet_json_1.default.abi, DiamondLoupeFacet_json_1.default.abi, OwnershipFacet_json_1.default.abi);
...
`);
    ...
    async function _deployViaDiamondProxy(name, options) {
        ...
        if (changesDetected) {
            ...
            if (!proxy) {
                const createReceipt = await execute(diamantaireName, options, "createDiamond", owner, cuts, data);
                if (!createReceipt) {
                ...
                if (options.log) {
                    log(`Diamond deployed at ${proxyAddress} via Diamantaire (${diamantaireDeployment.address}) cuts: ${cuts} with ${createReceipt.gasUsed} gas`);
                }
                proxy = {
                ...
                const executeReceipt = await execute(name, options, "diamondCut", cuts, "0x0000000000000000000000000000000000000000", data);
                if (!executeReceipt) {
