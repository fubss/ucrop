...
const construction_1 = require("@nomiclabs/buidler/internal/core/providers/construction");
const lazy_1 = require("@nomiclabs/buidler/internal/util/lazy");
const config_1 = require("@nomiclabs/buidler/config");
const errors_1 = require("@nomiclabs/buidler/internal/core/errors");
const server_1 = require("@nomiclabs/buidler/internal/buidler-evm/jsonrpc/server");
const constants_1 = require("@nomiclabs/buidler/internal/constants");
const types = __importStar(require("@nomiclabs/buidler/internal/core/params/argumentTypes"));
const errors_list_1 = require("@nomiclabs/buidler/internal/core/errors-list");
...
function isBuidlerEVM(bre) {
    const { network, buidlerArguments, config } = bre;
    return !(network.name !== constants_1.BUIDLEREVM_NETWORK_NAME &&
...
function default_1() {
    ...
    config_1.internalTask(task_names_1.TASK_COMPILE_GET_COMPILER_INPUT).setAction(async (_, __, runSuper) => {
    ...
    config_1.task("deploy", "Deploy contracts")
        .addOptionalParam("export", "export current network deployments")
        .addOptionalParam("exportAll", "export all deployments into one file")
        .addOptionalParam("tags", "dependencies to run")
    ...
    config_1.task("export", "export contract deployment of the specified network into one file")
        .addOptionalParam("export", "export current network deployments")
        .addOptionalParam("exportAll", "export all deployments into one file")
        .setAction(async (args, bre) => {
    ...
    config_1.task(task_names_1.TASK_NODE, "Starts a JSON-RPC server on top of Buidler EVM")
        .addOptionalParam("export", "export current network deployments")
        .addOptionalParam("exportAll", "export all deployments into one file")
        .addOptionalParam("tags", "dependencies to run")
        .addOptionalParam("write", "whether to write deployments to file", true, types.boolean)
        ...
        await bre.run("deploy:run", args);
        // enable logging
        const provider = bre.network.provider;
        ...
        try {
            ...
            if (watchRequired) {
                try {
                    ...
                    await watchCompilerOutput(server.getProvider(), config.solc, config.paths);
                }
                catch (error) {
        ...
        if (watch) {
            await bre.run("deploy:run", Object.assign(Object.assign({}, args), { watchOnly: true }));
        }
        await server.waitUntilClosed();
    });
    config_1.task("etherscan-verify", "submit contract source code to etherscan")
        ...
        .addFlag("solcInput", "fallback on solc-input (useful when etherscan fails on the minimum sources)")
        .setAction(async (args, bre, runSuper) => {
        ...
        await etherscan_1.submitSources(bre, solcInputsPath, {
            etherscanApiKey,
